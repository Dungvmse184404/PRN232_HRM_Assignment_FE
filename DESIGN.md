...
>   (không có tiền tố `harvest-`/`-black`/`-white` như bảng dưới).
> - Font thực dùng là **Inter** + **Playfair Display**, không phải MuotoWeb/Monarch (2 font đó
>   là font thương mại, dự án dùng bản thay thế đã ghi ở mục *Substitute*).
> - Khu vực sau đăng nhập (`.app-shell`) và trang chủ chạy **tông tối** obsidian/crimson/gold.
>   Bảng màu sáng bên dưới giờ chỉ còn là giá trị mặc định của token trước khi bị `.app-shell`
>   trỏ lại. Xem `README.md` mục *Giao diện*.
